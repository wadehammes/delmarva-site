...
- **County boundaries**: **`POST /api/boundaries/counties`** accepts up to **100** county names (max **120** characters each), resolves polygons from cached US GeoJSON first, then Mapbox geocoding (`country=US`) as fallback. Helpers live in [countyBoundaryHelpers.ts](../../src/app/api/boundaries/countyBoundaryHelpers.ts). Limits **`MAX_COUNTIES_PER_REQUEST`** and **`MAX_COUNTY_NAME_LENGTH`** are defined in [countyBoundaryLimits.ts](../../src/utils/countyBoundaryLimits.ts) and shared by server validation and client chunking.
- **Client chunking**: [countyUtils.ts](../../src/utils/countyUtils.ts) splits large service county lists into sequential batches and merges features before Mapbox layers are added. Services with **>100** counties depend on this—raise the API cap only when both server validation and client chunking are updated together.
- **Map UI**: Loading overlay, legend, and layer wiring in [AreasServicedMap](../../src/components/AreasServicedMap/) — see [components.md](components.md).
...
- **Routes**: [`src/app/api/resend/`](../../src/app/api/resend/) (Join Our Team, General Inquiry, Request a Proposal). Notification **`to`** / **`bcc`** addresses are resolved server-side from Contentful via **`formId`** — see [formNotificationRecipients.ts](../../src/lib/formNotificationRecipients.ts).
