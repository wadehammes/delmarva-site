...
| **`VERCEL_API_TOKEN`** | Server-only personal access token for polling deploy status on the refresh-content page ([vercelDeploymentStatus.ts](../../src/lib/vercelDeploymentStatus.ts)). **Required** on Preview/Production (and **Development** for local **`pnpm dev`**) for live completion toasts—without it the button shows **Deploy triggered — live status unavailable** and still fires **Refresh may be complete** after **`DEPLOY_ESTIMATED_MS`** (2 minutes). Set via **`vercel env add`**; also set **`VERCEL_TEAM_SLUG=worldwadeweb`** on **Production**, **Preview**, and **Development** (List Deployments returns **404** without team scope). **`VERCEL_TEAM_ID`** is injected automatically on Vercel and is tried first; slug is the fallback. **Redeploy** after adding or changing these vars so running deployments pick them up. |
...
- **Deploy monitoring**: After a hook trigger, **`POST /api/refresh-content/deploy`** returns **`createdAt`**, **`deployHookId`**, and **`projectId`**. Deploy route handlers share **`parseDeployTarget`**, **`parseDeployHookUrl`**, and **`resolveDeployHookMetadata`** from [refreshContentAccess.ts](../../src/lib/refreshContentAccess.ts); [vercelDeploymentStatus.ts](../../src/lib/vercelDeploymentStatus.ts) handles Vercel List Deployments polling only. [useDeployMonitor.ts](../../src/hooks/useDeployMonitor.ts) (used by [DeployButton.component.tsx](../../src/components/DeployButton/DeployButton.component.tsx)) calls **`api.deploy.*`** via React Query: a **mutation** for trigger, **polled queries** for **`GET /api/refresh-content/deploy/status`**, and an **active** query for **`GET /api/refresh-content/deploy/active`** on load. [DeployButton.component.tsx](../../src/components/DeployButton/DeployButton.component.tsx) also queries **`GET /api/refresh-content/deploy/build-stats`** ([useDeployBuildStats.query.ts](../../src/hooks/queries/useDeployBuildStats.query.ts)) via [DeployBuildStats.component.tsx](../../src/components/DeployPage/DeployBuildStats.component.tsx) to show one centered average build time from the last five **READY** project deployments (**`ready - buildingAt`**, any hook) under the refresh buttons. **`startedAt`** (button click time) and hook **`createdAt`** (job timestamp from the trigger response) are persisted in **`localStorage`**; status polling sends both as **`since`** and **`jobCreatedAt`**. The **status**, **active**, and **build-stats** route handlers are **`force-dynamic`** and return **`Cache-Control: no-store`**; [vercelDeploymentStatus.ts](../../src/lib/vercelDeploymentStatus.ts) uses **`cache: 'no-store'`** on Vercel API fetches and matches hook deployments **client-side** on the last-20 list (do **not** pass Vercel’s **`since`** query param—it can exclude the deployment). Matching prefers **`meta.deployHookId`**, falls back to **`source: git-deploy-hook`**, picks the **newest** deployment in a **2-minute** window anchored on click and job time, and treats failed List Deployments responses as **`monitoring: false`** (not endless **`pending`**). List Deployments tries **`VERCEL_TEAM_ID`** first, then **`VERCEL_TEAM_SLUG`**. When **`monitoring: false`**, the button re-enables immediately with a **Deploy triggered — live status unavailable** toast and still shows **Refresh may be complete** after **`DEPLOY_ESTIMATED_MS`**. Client **`api.deploy.status`** / **`api.deploy.active`** requests also pass **`cache: 'no-store'`**. Status polling uses the Vercel [List Deployments](https://vercel.com/docs/rest-api/deployments/list-deployments) API via **`VERCEL_API_TOKEN`**. Polling stops when the status is terminal (**`ready`**, **`error`**, **`canceled`**) or when **`monitoring: false`** (no API token or List Deployments error). When the token is unset, the button label still counts up for **`DEPLOY_ESTIMATED_MS`** (2 minutes) and the elapsed timer shows **Refresh may be complete**—without repeated status polls. Completion toasts (**Refresh complete**, **Refresh may be complete**, failure/timeout) re-check every second from the elapsed timer as well as on status poll updates—no page refresh required. The button label uses a monotonic **`displayStatus`** (via **`furthestDeployStatus`** in [deployProgressStorage.ts](../../src/lib/deployProgressStorage.ts)) so the phase only moves forward—**Starting** → **Queued**/**Building**—and shows status with elapsed time (**`Starting (m:ss)`**, **`Building (m:ss)`**, etc.) until a terminal toast.
