...
import { useClientLocale } from "src/i18n/getClientLocale";
  ...
  if (isSubmitSuccessful) {
    return (
      <div className={styles.formSubmitSuccess}>
        <RichText document={fields.formSubmitSuccessMessage} />
      </div>
    );
  }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
        ...
        render={({ field: { onChange, value, name, ref } }) => (
          <div className={styles.checkboxWrapper}>
            <input
              ...
              type="checkbox"
            />
            <label className={styles.checkboxLabel} htmlFor={workEligibilityId}>
      ...
      <div className={styles.formSubmitContainer}>
        <div>{hasMissingFields ? <p>{t.messages.missingFields}</p> : null}</div>
        <div>
          <Button
